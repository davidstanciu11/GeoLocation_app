...
class Map {
  ...
  constructor() {
    ...
    activitiesContainer.addEventListener("click", (e) => {
      ...
      if (!trashBin) this._moveToPop(e);
      else {
        const taskEl = e.target.closest(".tasks");
        if (!taskEl) return;
        this._deleteTask(taskEl.dataset.id);
      }
    });
  }
  ...
  _renderTask(activity) {
    ...
    if (activity.type === "jogging") {
      html += `<li class="tasks task_jogging" data-id="${activity.id}">
							...
							<span class="workout__delete">🗑</span></div>
							<div class="task_flex">
  ...
  _moveToPop(e) {
    ...
    const activity = this.#activities.find(
      (task) => task.id === activityEl.dataset.id
    );
  ...
  _deleteTask(id) {
    ...
    this.#activities.forEach((task, i) => {
      if (task.id === id) {
